...
public class Teenager implements Serializable{
    ...
    public String getRedhibitoire(Teenager teenager) {
        if (this.problemeAllergie(teenager)) return "Allergie";
        if (!this.peutNourrir(teenager)) return "Régime alimentaire";
        if (this.country == Country.FRANCE || teenager.getCountry() == Country.FRANCE) {
            if (this.nbLoisirCommun(teenager) == 0) return "Loisirs communs";
        }
        return "Aucune";
    }
    ...
    public String toViewString() {
        return ID + "\t|\t" + firstname + " " + name + " (" + country + ")";
    }
    ...
    /**
     * Permet de savoir si le visiteur ou l'hôte son compatible par rapport à la préférence du genre
     * @param t
     * @return
     */
    public boolean pairGender(Teenager t){
        if(this.criterionEquals("GENDER", "other") && t.criterionEquals("PAIR_GENDER", "other")||
        t.criterionEquals("GENDER", "other") && this.criterionEquals("PAIR_GENDER", "other")){
            return true;
        }
        if(this.criterionEquals("GENDER", "male") && t.criterionEquals("PAIR_GENDER", "male")||
        t.criterionEquals("GENDER", "male") && this.criterionEquals("PAIR_GENDER", "male")){
            return true;
        }
        if(this.criterionEquals("GENDER", "female") && t.criterionEquals("PAIR_GENDER", "female")||
        t.criterionEquals("GENDER", "female") && this.criterionEquals("PAIR_GENDER", "female")){
            return true;
        }
        else{
            return false;
    ...
    /**
     * Permet de savoir si le visiteur et l'hôte on 1 an et demi d'écart
     * @param t
     * @return
     */
    public boolean birthdayPref(Teenager t){
        long i = ChronoUnit.DAYS.between(t.getBirthday(), this.getBirthday());
        boolean res = false;
        if(i >= -548 && i <= 548){
            res = true;
        }
    ...
    /**
     * Permet de récupérer un nom de famille
    ...
    /**
     * Permet de recupérer une date d'anniversaire
     * @return une date d'anniversaire
     */
    ...
    /**
     * Permet de configurer un nom de famille
     * @param firstname
     */
    ...
    /**
     * Permet de configurer une date d'anniversaire 
     * @param birthday
     */
    ...
    /**
     * Permet de configurer un critère et sa valeur
     * @param critName
     * @param value
     */
