...
public class Affectations implements Serializable {
    ...
    public static final String PATH = System.getProperty("user.dir") + File.separator + "res" + File.separator;
    ...
    public String toString() {
        ...
        for (Map.Entry<Teenager, Teenager> entry : this.affectations.entrySet()) {
            if(entry.getKey().compatibleWithGuest(entry.getValue())){
                result += entry.getKey().toShortString() + " héberge " + entry.getValue().toShortString() + " et ils ne possèdent pas de contraintes rédhibitoires" + "\n";
            }else{
                result += entry.getKey().toShortString() + " héberge " + entry.getValue().toShortString() + " mais ils possèdent une contrainte rédhibitoire" + "\n";
            }
    ...
    public String[] getListViewItems() {
        String[] result = new String[this.affectations.size()];
        int i = 0;
        for (Map.Entry<Teenager, Teenager> entry : this.affectations.entrySet()) {
            result[i] = entry.getKey().toShortString() + "\t->\t" + entry.getValue().toShortString();
            i++;
        }
        return result;
    }
